...
class DQN_CP_NETWORK(tf.keras.Model):
    def __init__(self, extractor, head_value, **kwargs):
        super(DQN_CP_NETWORK, self).__init__(**kwargs)
...
def get_DQN_CP_BASE_agent(env, args, writer=None):
    ...
    return DQN_CP_BASE(env, extractor_object_policy, head_value_policy, model_dynamics, step_plan_max=args.step_plan_max, gamma=args.gamma, steps_total=args.steps_max, ignore_model=args.ignore_model, embed_pos=embed_pos, writer=writer, value_min=args.value_min, value_max=args.value_max, reward_min=args.reward_min, reward_max=args.reward_max)

class DQN_CP_BASE(RL_AGENT):
    def __init__(self,
        env, extractor_object_policy, head_value_policy, model,
        step_plan_max=5, gamma=0.99, exploration_fraction=0.02, exploration_final_eps=0.01, epsilon_eval=0.001, steps_total=50000000, freq_record=512, ignore_model=False, clip_reward=False, embed_pos=None, writer=None, value_min=None, value_max=None, reward_min=None, reward_max=None):

        super(DQN_CP_BASE, self).__init__(env, gamma, writer, disable_record=writer is None)
        ...
        self.network_policy = DQN_CP_NETWORK(self.extractor_policy, self.head_value_policy, name='network_policy')
        ## model
        self.model = model
    ...
    def decide(self, obs, eval=False, disable_planning=False, env=None, suffix_record='', heuristic='best_first', record_ts=True):
        ...
        if np.random.rand() > epsilon:
            flag_record_decide = not self.disable_record and record_ts and (self.steps_interact - self.step_last_record_decide) >= self.freq_record
...
def get_DQN_CP_agent(env, args, replay_buffer=None, writer=None):
...
class DQN_CP(DQN_CP_BASE):
    def __init__(self,
        ...
        clip_reward=False, ignore_TD=False, ignore_model=False, embed_pos=None, writer=None,
        value_min=None, value_max=None, reward_min=None, reward_max=None):

        super(DQN_CP, self).__init__(env, extractor_object_policy, head_value_policy, model,
        ...
        if self.extractor_target is not None and self.head_value_target is not None:
            self.network_target = DQN_CP_NETWORK(self.extractor_target, self.head_value_target, name='network_target')
