...
from utils import to_categorical, from_categorical, embed_pos_hd, huber_from_L1, add_timing_signal_nd, LinearSchedule, clip_gradients
...
def get_DQN_NOSET_BASE_agent(env, args, writer=None):
    ...
    head_value_policy = ESTIMATOR_VALUE_NOSET(num_actions=env.action_space.n, value_min=args.value_min, value_max=args.value_max, atoms=args.atoms_value)
    if args.disable_bottleneck: args.size_bottleneck = extractor_object_policy.m
    if args.ignore_model:
        model_dynamics = None
    else:
        model_dynamics = MODEL_TRANSITION_NOSET(n_action_space=env.action_space.n, len_action=args.len_ebd_action, layers_model=args.layers_model, reward_min=args.reward_min, reward_max=args.reward_max, atoms_reward=args.atoms_reward, norm=args.layernorm, signal_predict_action=args.signal_predict_action, len_hidden=args.len_hidden)
...
def get_DQN_NOSET_agent(env, args, replay_buffer=None, writer=None):
    ...
    head_value_target = ESTIMATOR_VALUE_NOSET(num_actions=env.action_space.n, value_min=args.value_min, value_max=args.value_max, atoms=args.atoms_value)
