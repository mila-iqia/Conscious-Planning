...
from components_CP import OBJECT_EXTRACTOR, ESTIMATOR_VALUE, MODEL_TRANSITION
from tree_search import best_first_search, random_search

class DQN_WM_NETWORK(tf.keras.Model):
    def __init__(self, extractor, head_value, **kwargs):
        super(DQN_WM_NETWORK, self).__init__(**kwargs)
...
def get_DQN_WM_BASE_agent(env, args, writer=None):
    ...
    head_value_policy = ESTIMATOR_VALUE(len_feature=args.len_feature, embed_pos=embed_pos, num_actions=env.action_space.n, value_min=args.value_min, value_max=args.value_max, norm=args.layernorm, atoms=args.atoms_value, noisy_shift=args.noisy_shift, n_head=args.n_head)
    if args.disable_bottleneck: args.size_bottleneck = extractor_object_policy.m
    if args.ignore_model:
        model_dynamics = None
    else:
        model_dynamics = MODEL_TRANSITION(len_feature=args.len_feature, embed_pos=embed_pos, n_action_space=env.action_space.n, len_action=args.len_ebd_action, n_head=args.n_head, layers_model=args.layers_model, m=extractor_object_policy.m, n=args.size_bottleneck, reward_min=args.reward_min, reward_max=args.reward_max, atoms_reward=args.atoms_reward, norm=args.layernorm, noisy_shift=args.noisy_shift, FC_depth=args.FC_depth, FC_width=args.FC_width, signal_predict_action=args.signal_predict_action, type_compress=args.type_compress)
    return DQN_WM_BASE(env, extractor_object_policy, head_value_policy, model_dynamics, step_plan_max=args.step_plan_max, gamma=args.gamma, steps_total=args.steps_max, ignore_model=args.ignore_model, period_warmup=args.period_warmup, embed_pos=embed_pos, writer=writer, value_min=args.value_min, value_max=args.value_max, reward_min=args.reward_min, reward_max=args.reward_max)

class DQN_WM_BASE(RL_AGENT):
    def __init__(self,
        env, extractor_object_policy, head_value_policy, model,
        step_plan_max=5, gamma=0.99, exploration_fraction=0.02, exploration_final_eps=0.01, epsilon_eval=0.001, steps_total=50000000, freq_record=512, ignore_model=False, period_warmup=0, clip_reward=False, embed_pos=None, writer=None, value_min=None, value_max=None, reward_min=None, reward_max=None):
        super(DQN_WM_BASE, self).__init__(env, gamma, writer)
        self.embed_pos = embed_pos
        self.period_warmup = int(period_warmup)
        ...
        self.network_policy = DQN_WM_NETWORK(self.extractor_policy, self.head_value_policy, name='network_policy')
        ## model
        self.model = model
    ...
    def initialize(self, obs_curr, action):
        ...
        features_curr = self.extractor_policy(obs_curr)
        self.network_policy(obs_curr)
        if not self.ignore_model:
            features_imagined, _, _, _ = self.model(features_curr, action)
            if self.model.signal_predict_action:
                if self.model.conscious:
                    _ = self.model._predict_action_subset(tf.zeros([1, self.model.n, self.model.len_object]), tf.zeros([1, self.model.n, self.model.len_object]))
                else:
                    _ = self.model._predict_action(features_curr, features_imagined)
...
def get_DQN_WM_agent(env, args, replay_buffer=None, writer=None):
    ...
    head_value_target = ESTIMATOR_VALUE(len_feature=args.len_feature, embed_pos=embed_pos, num_actions=env.action_space.n, value_min=args.value_min, value_max=args.value_max, norm=args.layernorm, atoms=args.atoms_value, noisy_shift=args.noisy_shift, n_head=args.n_head)
...
class DQN_WM(DQN_WM_BASE):
    def __init__(self,
        ...
        clip_reward=False, ignore_TD=False, ignore_model=False, period_warmup=0, embed_pos=None, writer=None,
        value_min=None, value_max=None, reward_min=None, reward_max=None, freeze_encoder_after_warmup=True, freeze_model_after_warmup=True):

        super(DQN_WM, self).__init__(env, extractor_object_policy, head_value_policy, model,
        step_plan_max=step_plan_max, gamma=gamma, exploration_fraction=exploration_fraction, exploration_final_eps=exploration_final_eps, epsilon_eval=epsilon_eval, steps_total=steps_total, ignore_model=ignore_model, embed_pos=embed_pos, clip_reward=clip_reward, writer=writer, value_min=value_min, value_max=value_max, reward_min=reward_min, reward_max=reward_max, period_warmup=period_warmup)

        self.replay_buffer = replay_buffer
        self.freeze_encoder_after_warmup, self.freeze_model_after_warmup = freeze_encoder_after_warmup, freeze_model_after_warmup
        ...
        if self.extractor_target is not None and self.head_value_target is not None:
            self.network_target = DQN_WM_NETWORK(self.extractor_target, self.head_value_target, name='network_target')
        ...
        if type_optimizer == 'Adam':
            ...
            if not self.ignore_model: self.optimizer_model = tf.keras.optimizers.Adam(learning_rate=lr, epsilon=eps)
            ...
            if not self.ignore_model: self.optimizer_model = tf.keras.optimizers.RMSprop(learning_rate=lr, epsilon=eps)
        ...
        self.step_last_update_TD = np.inf if self.ignore_TD else self.time_learning_starts - self.freq_train_TD + self.period_warmup
    ...
    def step_update(self, batch=None):
        ...
        if self.steps_interact >= self.time_learning_starts:
            ...
            flag_train_model = not self.ignore_model and (self.steps_interact - self.step_last_update_model) >= self.freq_train_model and not (self.freeze_model_after_warmup and self.steps_interact >= self.period_warmup)
            if flag_train_TD or flag_train_model:
                self.update(flag_train_TD, flag_train_model, batch=batch)
            if not self.ignore_TD and (self.steps_interact - self.step_last_targetnet_update) >= self.freq_targetnet_update and self.steps_interact >= self.period_warmup:
                self.sync_parameters()
                self.step_last_targetnet_update += self.freq_targetnet_update
        if self.steps_interact >= self.period_warmup:
            self.steps_processed = self.step_last_update_TD
        else:
            self.steps_processed = self.step_last_update_model

    def need_update(self):
        if not self.initialized and self.replay_buffer.get_stored_size() >= self.size_batch: return True
        if self.steps_interact >= self.time_learning_starts:
            flag_train_TD =  self.steps_interact >= self.period_warmup and not self.ignore_TD and (self.steps_interact - self.step_last_update_TD) >= self.freq_train_TD
            flag_train_model = not self.ignore_model and (self.steps_interact - self.step_last_update_model) >= self.freq_train_model and not (self.freeze_model_after_warmup and self.steps_interact >= self.period_warmup)
    ...
    def update(self, flag_train_TD=True, flag_train_model=True, batch=None):
        ...
        if not self.flag_optimizers_initialized or flag_train_TD and self.steps_interact > self.period_warmup:
        ...
        if not flag_train_model:
            ...
            if batch is None: batch = self.sample_batch()
            batch_obs_curr, batch_action, batch_reward, batch_obs_next, batch_done, batch_not_done, weights, batch_idxes = batch
            gradients_model, error_reward_imagined_L1_weighted, tp_term_imagined, fn_term_imagined, acc_term_imagined, acc_action_imagined, error_dynamics_L1_changed_relative, error_dynamics_L1_unchanged_relative, error_value_consistency_L1_weighted, error_model_weighted, error_dynamics_weighted, error_term_imagined_weighted, error_reward_imagined_weighted, error_action_imagined_weighted, error_value_consistency_weighted, cosdist_features, norm_features_L1, error_dynamics_L1_relative = self._update_model(batch_obs_curr, batch_action, batch_obs_next, batch_done, batch_not_done, batch_reward, batch_features_next_policy, Q_dist_next_policy, weights, flag_record=flag_record)
            if flag_train_TD:
                self._apply_gradients_TD(gradients_TD, clip_TD=self.clip_gradient_TD)
                self.step_last_update_TD += self.freq_train_TD
            ...
            if flag_record:
                ...
                if flag_train_model:
                    ...
                    self.record_scalar('Debug/consistency_value', error_value_consistency_weighted, self.step_last_update_model)
    ...
    def _update_TD(self, batch_obs_curr, batch_obs_next, batch_action, batch_reward, batch_not_done, weights, flag_record=True):
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:
            ...
            batch_features_curr = self.extractor_policy(batch_obs_curr)
            if self.freeze_encoder_after_warmup: batch_features_curr = tf.stop_gradient(batch_features_curr)
            Q_logits_curr = self.head_value_policy(batch_features_curr, softmax=False, eval=True)
            V_dist_curr = tf.nn.softmax(tf.gather_nd(Q_logits_curr, indices), axis=-1)
    ...
    def _update_model_forward(self, batch_obs_curr, batch_action, batch_done, batch_reward_categorical, batch_features_next):
        batch_features_curr = self.extractor_policy(batch_obs_curr)
        if self.freeze_encoder_after_warmup: batch_features_curr = tf.stop_gradient(batch_features_curr)
        batch_features_imagined, batch_reward_dist_imagined, batch_done_logits_imagined, batch_action_logits_imagined = self.model.forward_train(batch_features_curr, batch_action)
        ...
        error_dynamics_L1 = tf.math.abs(tf.stop_gradient(batch_features_next) - batch_features_imagined)
        ...
        return error_model_weighted, error_term_imagined_weighted, error_reward_imagined_weighted, error_action_imagined_weighted, error_dynamics_weighted, batch_reward_dist_imagined, batch_done_logits_imagined, batch_action_logits_imagined, error_dynamics_L1, batch_features_imagined
    ...
    def _update_model(self, batch_obs_curr, batch_action, batch_obs_next, batch_done, batch_not_done, batch_reward, batch_features_next, Q_dist_next, weights, flag_record=False):
        ...
        if flag_record:
            ...
            if 'minigrid' in self.extractor_policy.type_env:
                if not self.extractor_policy.features_learnable:
            ...
            Q_dist_imagined = self.head_value_policy(batch_features_imagined, eval=True)
            ...
            Q_next = from_categorical(Q_dist_next, value_min=self.value_min, value_max=self.value_max, atoms=self.head_value_policy.atoms)
            Q_imagined_next = from_categorical(Q_dist_imagined, value_min=self.value_min, value_max=self.value_max, atoms=self.head_value_policy.atoms)
    ...
    def initialize(self):
        ...
        batch_features_curr, _ = self.extractor_policy(batch_obs_curr), self.extractor_target(batch_obs_curr)
        ...
        if self.freeze_encoder_after_warmup:
            self.parameters_train_TD = self.head_value_policy.trainable_variables
        else:
            self.parameters_train_TD = self.network_policy.trainable_variables
        if not self.ignore_model:
            batch_features_imagined, _, _, _ = self.model(batch_features_curr, batch_action)
            if self.model.signal_predict_action:
                if self.model.conscious:
                    _ = self.model._predict_action_subset(tf.zeros([1, self.model.n, self.model.len_object]), tf.zeros([1, self.model.n, self.model.len_object]))
                else:
                    _ = self.model._predict_action(batch_features_curr, batch_features_imagined)
            if self.freeze_encoder_after_warmup:
